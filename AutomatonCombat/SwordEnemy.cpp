...
SwordEnemy::SwordEnemy(Player* player,NavMeshAstar *astar)
{
	...
	s_object = std::make_unique<SwordEnemyObject>();

	s_object->swordObject = std::make_unique<Object3d>();
	s_object->swordObject->SetLoadedModelData(swordModel);
	s_object->player = player;
	m_swordWaitJudge.reset(new Sword_WaitJudge(s_object.get()));
	m_swordWaitAct.reset(new Sword_WaitAct(s_object.get()));
	m_swordAttackJudge.reset(new Sword_AttackJudge(s_object.get(), m_swordWaitAct.get()));
	m_swordAttackAct.reset(new Sword_AttackAct(s_object.get()));
	m_approachJudge.reset(new Sword_ApproachJudge(s_object.get()));
	m_approahAct.reset(new Sword_ApproachingAct(s_object.get(),astar));
	m_chargeJudge.reset(new Sword_ChargeJudge(s_object.get()));
	m_chargeAct.reset(new Sword_ChargeAct(s_object.get()));

	rootNode.reset(new BehaviorBaseNode);
	actNode.reset(new BehaviorBaseNode);
	waitNode.reset(new BehaviorBaseNode);
	approachNode.reset(new BehaviorBaseNode);
	chargeNode.reset(new BehaviorBaseNode);
